...
public class MascotasController {
    @Autowired
    private MascotasService userService;//principio de Inversión de Dependencias (IoD)
    ...
    public ResponseEntity<?> readOne(@PathVariable(value = "id") Long id) {
        Optional<Mascotas> oUser = userService.findById(id);
    ...
    public ResponseEntity<?> update(@RequestBody Mascotas userDetails, @PathVariable(value = "id") Long id) {
        Optional<Mascotas> user = userService.findById(id);
        if (!user.isPresent()) {
            return ResponseEntity.notFound().build();
        }
    ...
    public List<Mascotas> readAll() {
        List<Mascotas> users = StreamSupport//<--hereda de Object y me trae los stream
